...
function paid(payment) {
    ...
    payment.forEach((status) => {
        // if (status.payment == 'Pending') {
        //     paidMoney += parseInt(status.amount)
        // }
...
const getData = (result) => {
    ...
    result.map((newArr) => {
        ...
        newArr.forEach((res) => {
            const collectionMonth = new Date(res.registration_date);
    ...
    function renderChart() {
        ...
        var chart = new ApexCharts(document.querySelector("#directorChart"), options);
        chart.render();
